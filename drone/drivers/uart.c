...
int _write(int file, const char * p_char, int len)
{
	...
	for (i = 0; i < len; i++)
	{
		uart_put(*p_char++);		// Disable prinf temporary
	}
	return len;
}
...
void UART0_IRQHandler(void)
{
	...
	if (NRF_UART0->EVENTS_TXDRDY != 0)
	{
		...
		if (uart_queue.count) NRF_UART0->TXD = dequeue(&uart_queue);
...
void uart_init(void)
{
	...
	init_queue(&uart_queue);		// Initialize uart (internal) queue
